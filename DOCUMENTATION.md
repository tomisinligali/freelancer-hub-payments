...
**What is in the slice.** Freelancer Hub's full payment/subscription layer: one product (the full plan), two intervals — Monthly ₦5,000, Yearly ₦50,000 — via Flutterwave hosted checkout (test mode). A signed-in Free user picks a plan, pays on Flutterwave's page, returns to a verified plan; the billing page shows plan, price, renewal date, and upgrade/downgrade/cancel. Built for real-money failure: amounts in kobo (never fractional naira); plan applied only after **server-side re-verification** against Flutterwave's API; webhook HMAC-verified against a shared secret; a payment can never apply twice (idempotency keyed on the transaction reference); append-only payment log that reads like a dispute ledger; prorated upgrades (you keep your current month's unused value, shown before confirming); cancellation keeps access until your paid period ends (reason recorded); a cancelled user can schedule a subscription that charges before it starts; payment endpoints rate-limited per user. Reuse from Assessment 1: per the brief, auth is reused unchanged — `auth()` session (`src/lib/auth/auth.ts`), `/auth` sign-in, dashboard auth guard — stated plainly, not hidden; the user table gains payment columns, the identity/session layer is untouched.

**What is deliberately not added.** The paying *work* (no clients, projects, invoices, time-tracking — the paywalled product is still out of scope; this slice sells a plan flag, nothing more), real (non-test) money movement, refunds/chargebacks, receipt emails, and billing automation beyond this document. Same scope discipline as the auth slice — a trust boundary small enough to verify end to end. The guarantee is narrow but absolute: **a paid plan is added only when a verified payment for the exact quoted amount reaches the server, a payment can never apply twice, and the payment log can never be altered after the fact.**
...
- **Server** (`return/page.tsx` → `service.processReturn`): logs the unverified claim (`PROVIDER_RETURN`), checks idempotency (if the session is already `COMPLETED` it reports "already active" — a completed payment is never processed twice), sanitizes the `transaction_id` (digits only), then **re-verifies with Flutterwave's API** (`provider.verifyTransaction` — to Flutterwave, not the browser it came from). If the verified status is `successful` **and** the verified amount equals our recorded kobo amount **and** the currency is NGN, it fulfils (Step 5) and the page reports "your plan is active". A failure or pending status reports accordingly and changes nothing. This is the behaviour the PRD prescribes (§6: "on return, the transaction is verified against the provider before any subscription state changes").

**Step 4 — The webhook arrives**
- **Flutterwave:** POSTs the charge result to `/api/webhooks/flutterwave`.
- **Server** (`webhooks/flutterwave/route.ts`): reads the raw body, recomputes `HMAC-SHA256(body, FLW_WEBHOOK_HASH)` (base64) and compares it to the `flutterwave-signature` header with `timingSafeEqual` — a mismatch is a **401** before the body is even parsed. Valid payloads are logged (`WEBHOOK_RECEIVED`, unverified), then `service.finalizeFromWebhook` runs the same verified, idempotent path as the return page.

**Step 5 — Fulfilment (where a plan is applied)**
- **Server** (`resolvePayment` in `subscription-service.ts`): the verification gate — idempotency check (COMPLETED → `ALREADY_COMPLETED`), provider re-verify, success + kobo amount match + NGN — runs identically on both the return page and the webhook. On success it does one `$transaction`: sets `user.plan` to the paid plan, `upsert`s the `Subscription` row (plan, `status: ACTIVE`, `currentPeriodStart: now`, `currentPeriodEnd: now + months(plan)`; for an **UPGRADE** the period is reset to a fresh 12 months), and marks the `CheckoutSession` `COMPLETED` (storing the provider's transaction id and subscription id). It then writes the `PAYMENT_VERIFIED`/`UPGRADE_PAID`/`RESUBSCRIBE_APPLIED` event and the `STATE_CHANGED` payment log — the ledger's record of "plan was granted, this is why." Both channels fulfil; the idempotency gate makes a second delivery (webhook after return, or vice-versa) a harmless `ALREADY_COMPLETED`.
...
**Why it is needed.** If the same code both recorded the payment and granted the plan, the return URL (which anyone can forge, submit twice, or replay) would be the authority on money. Payments must be *initiated* on our side so we know exactly what we quoted, *verified* against the provider so we never trust the browser, and *fulfilled* only on a positive, amount-matching provider verification — on either the return page *or* the webhook, so a closed tab or a forged redirect can't block a genuine grant, and a forged redirect can't start one (the return URL grants nothing until the server re-check succeeds).

**How I implemented it.** `initiateCheckout` (initiation) computes the kobo amount, mints `txRef`, writes `CheckoutSession` + `CHECKOUT_INITIATED` log, and returns the hosted URL. `resolvePayment` (verification + fulfilment) logs the unverified claim, re-verifies via `provider.verifyTransaction`, checks status/amount/currency, and — **only after a positive, amount-matching verification** — applies the plan in one `$transaction` and logs `STATE_CHANGED`. Both the return page and the webhook call the same resolver through different entry points (`processReturn` / `finalizeFromWebhook`). The return page gives the user instant feedback; the webhook is the backstop when a user never returns (closed the tab, network failure), logging the delivery either way.

**What I chose against, and why.** Trusting the *unverified* return (a forged redirect must fail the provider re-verify and amount/currency gates before any state change — the redirection itself grants nothing), trusting webhook bodies without re-verifying (the webhook can be replayed or spoofed — Section 5 concept 5), and granting when the verified amount differs from the quoted amount (would let a smaller payment buy the plan). The lifecycle — initiation lowers the exact quote, verification re-checks it against the provider, fulfilment applies only a verified, amount-matching payment — is why a payment is applied exactly once, at exactly the price quoted, whether the confirmation arrives by return or webhook.
...
**Why it is needed.** The webhook is one of two paths that can grant a plan (the other being the server-side re-verified return path, Section 3, Step 5) — the difference is that the webhook is fired by the provider and needs **no human return**, making it the highest-authority delivery channel. If an attacker could POST `{data: {tx_ref: …, status: "successful"}}` at that URL with no signature, they could grant themselves any plan without paying. The shared secret is known only to us and Flutterwave, so a correct signature *is* proof the payload came from the provider, and constant-time comparison (`timingSafeEqual`) stops a length/memory-timing leak from making brute-force feasible.
...
- **No retry queue for payments.** If the return page confirmation is missed (tab closed, network dropped) and the webhook never arrives and the reconcile sweep also misses a window, a genuine payment waits for the next sweep to be applied. Acceptable for one merchant in test mode; wrong for production throughput.
...
The result for a finalized subscription looks like:
